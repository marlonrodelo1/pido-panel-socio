...
serve(async (req) => {
  ...
  if (asig.estado !== 'esperando_aceptacion') return jsonResponse({ error: 'estado_invalido', actual: asig.estado }, 409)
  await sb.from('pedido_asignaciones').update({ estado: 'rechazado', motivo_rechazo: body.motivo || null, resolved_at: new Date().toISOString() }).eq('id', asig.id)
