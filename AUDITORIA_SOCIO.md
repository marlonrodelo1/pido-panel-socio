...
- ~~**Shipday**: retirar las funciones muertas.~~ **HECHO (1-jul):** `shipday-webhook`, `sync-shipday-status`, `check-shipday-drivers`, `check-shipday-carriers-status` y `validar-shipday-key` convertidas en tombstones 410; cron `check-shipday-carriers-every-1min` eliminado. NO se tocó `create-shipday-order` (es el dispatcher propio, solo comparte el nombre) ni las columnas `shipday_status`/`shipday_tracking_url` (reutilizadas). Si se reactivara Shipday, habría que reponer y configurar `SHIPDAY_WEBHOOK_TOKEN`.
- ~~**Rendimiento RLS**: 108 políticas reevalúan `auth.uid()` por fila.~~ **HECHO (1-jul):** migración `rls_wrap_auth_uid_initplan` envolvió las 108 políticas (`auth.uid()`→`(select auth.uid())`, ídem role/jwt/email); verificado 0 restantes y que el control de acceso sigue intacto (el cliente solo ve sus pedidos). **Pendiente opcional:** consolidar las políticas permisivas duplicadas (`multiple_permissive_policies`, 142 en tablas activas). Es una reestructuración masiva de bajo beneficio a esta escala y el dueño ya está haciéndola manualmente (migraciones `perf_rls_dedupe_*`); se deja a su criterio para no pisar ese trabajo.
